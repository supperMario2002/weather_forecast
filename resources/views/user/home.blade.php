    ...
    <div class="container">
        <div class="row row-cols-1 row-cols-md-2 top-part-banner1">
            ...
            <div class="col-lg-6" style="padding: 0 15px;">
                <div class="time-day d-flex">
                    <h3>
                        <i class="fas fa-map-marker-alt"></i> {{ $nameLocation }}
                        <span class="d-block">{{ $currentDate->dayOfWeek == 7 ? 'Chủ nhật' : 'Thứ ' . ($currentDate->dayOfWeek + 1) }},
                            {{ $currentDate->format('d-m-Y') }}</span>
                    </h3>
                    <h3 class="ml-5" id="oclock">
            ...
            <div class="col-lg-6">
                ...
                <iframe width="100%" height="270" src="https://embed.windy.com/embed2.html?lat={{ $dataLocation['lat'] }}&lon={{ $dataLocation['lon'] }}&detailLat=16.299&detailLon=105.864&width=650&height=450&zoom=5&level=surface&overlay=wind&product=ecmwf&menu=&message=true&marker=true&calendar=now&pressure=&type=map&location=coordinates&detail=&metricWind=default&metricTemp=%C2%B0C&radarRange=-1" frameborder="0"></iframe>
            </div>
        </div>
...
<section class="body-part float-start w-100 mt-5">
    <div class="container" style="display: flex; flex-direction: column">
        <h1 class="wheather-group-title">Lời khuyên của AI</h1>
        <button id="gpt" class="btn btn-info">Đưa ra lời khuyên</button>
        <textarea class="" id="ms-gpt" cols="100" rows="10" style="background-color: #FFFFFF" readonly>
Dựa trên thông số nồng độ khí thải của các chất gây ô nhiễm trong không khí mà bạn cung cấp, đây là một số lời khuyên để bảo vệ sức khỏe con người:
1. Giảm thiểu thời gian ở nơi có mức độ ô nhiễm cao, đặc biệt là trong các khu vực đông dân, bên cạnh các con đường giao thông chính.
2. Sử dụng mặt nạ bảo vệ để hạn chế hít phải các chất ô nhiễm.
3. Đi bộ hoặc sử dụng phương tiện giao thông công cộng thay vì lái xe riêng để giảm lượng khí thải được thải ra môi trường.
4. Cung cấp thông gió tốt trong nhà để làm sạch không khí, đồng thời cũng nên vệ sinh nhà cửa thường xuyên để giảm bụi bẩn và các hạt nhỏ khác trong không khí.
5. Hạn chế tiếp xúc với các loại hóa chất khách trong môi trường như thuốc trừ sâu, chất tẩy rửa, chất khử trùng,... để tránh phản ứng hóa học với các chất ô nhiễm trong không khí và gây ra tác hại đối với sức khỏe con người.</textarea>
    </div>
</section>
...
<script>
    ...
    $("#gpt").click(function() {
        ...
        eventSource.onmessage = function(e) {
            ...
            if(JSON.parse(e.data).choices[0].text == ''){
                console.log(JSON.parse(e.data).choices[0].text);
            }
